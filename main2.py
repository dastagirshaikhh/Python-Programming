...
def reindent(
    buffer, tabs=2, tospaces=False, nospaces=True, fromtabs=None, backupsuffix="~"
):
    if fromtabs == None:
        fromtabs = tabs
    outlines = []
        ...
        for lineno, line in enumerate(lines):
            ...
            if indent - (indent // tabs) * tabs > 0:
                if nospaces and not tospaces:
                    ...
                    raise RuntimeError(
                        f"[line {lineno}] Indentation does not line up with tab-stops, aborting."
                    )
                else:
...
def reindent_file(filename, backupsuffix="~", backupcount=None, *args, **kwargs):
    ...
    if not backup_file(filename, backupsuffix=backupsuffix, nobackup=backupcount == 0):
        raise RuntimeError(
            f"Failed to backup original input file, refusing to overwrite: {filename}"
        )
...
if __name__ == "_main_":
    parser = argparse.ArgumentParser(
        description="Fix in-place python scripts with inconsistent indentation"
    )
    parser.add_argument(
        "-t",
        "--tabs",
        type=int,
        default="2",
        help="Output tab width in columns. Default is 2.",
    )
    parser.add_argument(
        "-s", "--space", action="store_true", help="Indent with spaces instead of tabs"
    )
    parser.add_argument(
        "-m",
        "--mixed",
        action="store_true",
        help="Allow indentation between tab stops by padding with spaces.",
    )
    parser.add_argument(
        "-f",
        "--from",
        ...
        type=int,
        default=None,
        help="Tab size in input to convert from. Defaults to -t output tab size.",
    )
    parser.add_argument(
        "-b",
        "--backup-suffix",
    ...
    parser.add_argument(
        ...
        type=str,
        default=-1,
        help="Maximum number of backups to keep. If negative, removes the oldest backup when count is reached. If positive, rotate backups and remove the oldest.",
    )
    parser.add_argument(
        "-c",
        "--cleanup",
        action="store_true",
        default=False,
        help="Automatically detect indented blocks in python mode and reindent each level by one block",
    )
    ...
    for fn in opts.filename:
        ...
        reindent_file(
            fn,
            backupsuffix=opts.backup_suffix,
            backupcount=opts.backup_count,
            tabs=opts.tabs,
            tospaces=opts.space,
            nospaces=not opts.mixed,
            fromtabs=opts.from_tabs,
        )
